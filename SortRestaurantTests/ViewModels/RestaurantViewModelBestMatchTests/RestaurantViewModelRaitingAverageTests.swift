...
class RestaurantViewModelRaitingAverageTests: XCTestCase {

    let defaultSort = RestaurantSort("Newest",RatingAverageSort())
    ...
    func testRaitingAverage_GroupedByOpenOrderCloseAndSorted() {
        ...
        waitForExpectations(timeout: 1) { error in
            ...
            XCTAssertTrue(viewModel[0].sort.ratingAverage >= viewModel[1].sort.ratingAverage)
            
            XCTAssertTrue(viewModel[2].status == .order)
            XCTAssertTrue(viewModel[3].status == .order)
            XCTAssertTrue(viewModel[2].sort.ratingAverage >= viewModel[3].sort.ratingAverage)
            
            XCTAssertTrue(viewModel[4].status == .close)
            XCTAssertTrue(viewModel[5].status == .close)
            XCTAssertTrue(viewModel[4].sort.ratingAverage >= viewModel[5].sort.ratingAverage)
            
        }
    }
    
    func testRaitingAverage_GroupedByOrderCloseAndSorted() {
    ...
    func testRaitingAverage_GroupedByOpenCloseAndSorted() {
    ...
    func testRaitingAverage_GroupedByOpenOrderAndSorted() {
    ...
    func testRaitingAverage_GroupedByFavouritesAndOpenOrderCloseAndSorted() {
